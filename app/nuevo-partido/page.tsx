		...
		try {
			...
			if (editingMatchId && existingMatch) {
				...
				if (newMatch && (penaltyShooters.length > 0 || rivalPenalties.length > 0)) {
					...
					const { error: penaltyError } = await supabase.from("penalty_shootout_players").insert([...penaltyPlayersToInsert]);
																...
																<div className="grid grid-cols-3 gap-2">
																	<Button
																		...
																		onClick={() =>
																			setRivalPenalties((prev) =>
																				prev.map((p) =>
																					p.id === penalty.id ? { ...p, result: "scored" } : p
																				)
																			)
																		}
																	...
																	<Button
																		...
																		onClick={() =>
																			setRivalPenalties((prev) =>
																				prev.map((p) =>
																					p.id === penalty.id ? { ...p, result: "missed" } : p
																				)
																			)
																		}
																		className={`flex flex-col items-center gap-1 h-auto py-2 ${
