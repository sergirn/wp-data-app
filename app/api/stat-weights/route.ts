...
export async function GET() {
	...
	const { data, error } = await supabase.from("user_stat_weights").select("stat_key, weight").eq("user_id", userId).eq("club_id", clubId);
...
export async function PUT(req: Request) {
	...
	if (toUpsert.length > 0) {
		const { error: upsErr } = await supabase.from("user_stat_weights").upsert(toUpsert, {
