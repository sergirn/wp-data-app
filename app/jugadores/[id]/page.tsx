...
function FieldPlayerSummary({ stats, matchCount, matchStats }: { stats: any; matchCount: number; matchStats: MatchStatsWithMatch[] }) {
  ...
  totalPenaltis > 0 ? ((stats.goles_penalti_anotado / totalPenaltis) * 100).toFixed(1) : "0.0"
...
const statsPerMatch = Array.isArray(matchStats)
  ? matchStats.map(({ matches, ...rest }) => rest)
  : []
...
function GoalkeeperSummary({ stats, matchCount, matchStats }: { stats: any; matchCount: number; matchStats: MatchStatsWithMatch[] }) {
  ...
  const penaltySaveRate =
    penaltiesAttempted > 0 ? ((stats.portero_paradas_penalti_parado / penaltiesAttempted) * 100).toFixed(1) : "0.0"
  ...
  const matches = Array.isArray(matchStats)
  ? matchStats.map((s) => s.matches).filter((m) => m?.id != null)
      .filter((m, i, arr) => arr.findIndex((x) => x.id === m.id) === i)
  : []
