...
export function DisciplineChart({ matches, stats }: Props) {
	const data = useMemo(() => {
		...
		return sorted.slice(-15).map((match, idx) => {
			...
			const exp_simple = sumField(ms, "faltas_exp_simple");
			const penalti = sumField(ms, "faltas_penalti");
			const contrafaltas = sumField(ms, "faltas_contrafaltas");
			const exp_trans_def = sumField(ms, "exp_trans_def");

			// ✅ exp_simple suma dentro del totalExclusiones (no va como barra aparte)
			const totalExclusiones = exp20_1c1 + exp20_boya + exp3_bruta + exp3_int + exp_simple + exp_trans_def;
								...
								content={
									<ChartTooltipContent
										labelFormatter={(label, payload) => {
											...
											return `${label} · vs ${p.rival} · ${p.fullDate} · Excl: ${p.totalExclusiones} (20"1c1:${p.exp20_1c1}, 20"b:${p.exp20_boya}, 3b:${p.exp3_bruta}, 3i:${p.exp3_int}, simple:${p.exp_simple}, trans_def:${p.exp_trans_def})`;
										}}
									/>
								}
							/>
							...
							<Table className="min-w-[1120px]">
								<UITableHeader className="sticky top-0 z-10 bg-card/95 backdrop-blur">
									<TableRow className="hover:bg-transparent">
										...
										<TableHead className="text-right">Exp trans def</TableHead>
								...
								<TableBody>
									{data.map((m, idx) => (
										<TableRow key={m.matchId} className={`${idx % 2 === 0 ? "bg-muted/20" : ""} hover:bg-muted/40`}>
											...
											<TableCell className="text-right tabular-nums">{m.exp_trans_def}</TableCell>
