...
function MiniBar({ leftLabel, leftValue, rightLabel, rightValue }: { leftLabel: string; leftValue: number; rightLabel: string; rightValue: number }) {
					...
					<div className={`grid gap-4 ${compact ? "grid-cols-1" : "grid-cols-1 md:grid-cols-2"}`}>
						...
						<div className="space-y-3">
							...
							<div className="flex flex-wrap gap-2 pt-1">
								<TinyPill>
									Total <span className="ml-1 font-semibold text-foreground tabular-nums">{computed.total}</span>
								</TinyPill>
								<TinyPill>
									Evitados <span className="ml-1 font-semibold text-foreground tabular-nums">{computed.evitados}</span>
								</TinyPill>
								<TinyPill>
									Recibidos <span className="ml-1 font-semibold text-foreground tabular-nums">{computed.recibidos}</span>
								</TinyPill>
							</div>

							{/* mini bar (lectura rápida) */}
							<div className="rounded-2xl border bg-card/40 p-3">
								<p className="text-xs font-semibold text-muted-foreground mb-2">Resultado (H-)</p>
								<MiniBar leftLabel="Evitados" leftValue={computed.evitados} rightLabel="Recibidos" rightValue={computed.recibidos} />
							</div>
				...
				<div className="rounded-2xl border bg-card/40 overflow-hidden">
					...
					<div className="p-4 space-y-3">
						<div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
							<Row label="Total intentos (Inf.-)" value={computed.total} />
