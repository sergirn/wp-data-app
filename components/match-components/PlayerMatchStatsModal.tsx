...
export function PlayerMatchStatsModal({ open, onOpenChange, player, stat, derived }: Props) {
  ...
  const goalsItems = [
    ...
    { label: "Hombre +", key: "goles_hombre_mas" },
  ...
  const missesItems = [
    { label: "Hombre +", key: "tiros_hombre_mas" },
      ...
      <DialogContent className="!w-[55vw] !max-w-none p-0 overflow-hidden">
        ...
        <div className="p-4 space-y-4 max-h-[80vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <KpiBox
              label="Goles"
              value={stat?.goles_totales ?? 0}
              className="bg-blue-500/5 border-blue-500/10"
            />
            <KpiBox
              label="Tiros"
              value={stat?.tiros_totales ?? 0}
              className="bg-white/5 border-blue-500/20"
            />
            <KpiBox
              label="Eficiencia"
              value={`${derived.shootingEfficiency}%`}
              className="bg-blue-500/5 border-blue-500/10"
            />
            <KpiBox
              label="Asistencias"
              value={stat?.acciones_asistencias ?? 0}
              className="bg-white/5 border-blue-500/20"
            />
          </div>
