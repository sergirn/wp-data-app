...
export function MatchSuperiorityChart({ stats }: { stats: SuperioridadStats }) {
  const computed = useMemo(() => {
    ...
    const intentos = anotadas + falladas
    ...
    // No te fías de stats.eficiencia? Recalc:
    const eficiencia = intentos > 0 ? Math.round((anotadas / intentos) * 1000) / 10 : 0

    const pctAnotadas = intentos > 0 ? Math.round((anotadas / intentos) * 1000) / 10 : 0
  ...
  return (
    <ExpandableChartCard
      title="Superioridad"
      description={`${computed.anotadas}/${computed.intentos} · ${computed.eficiencia}% · Reb ${computed.rebRec}/${computed.rebPer}`}
            ...
            <div className={`${compact ? "h-[220px]" : "h-[320px]"} w-full`}>
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    data={[
                      { name: "Anotadas", value: computed.anotadas },
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 border-b bg-muted/20 px-4 py-3">
            <div className="min-w-0">
              ...
              <p className="text-xs text-muted-foreground">
                Eficiencia recalculada · {computed.anotadas}/{computed.intentos} · {computed.eficiencia}%
              </p>
            </div>
