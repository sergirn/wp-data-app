...
import { getPlayerDerived, n } from "@/lib/stats/playerStatsHelpers";

export function PlayerStatsCard({ stat, player }: { stat: MatchStats; player: Player }) {
	const [open, setOpen] = useState(false);

	const derived = getPlayerDerived(stat as Record<string, any>);
					...
					<div className="p-2.5 sm:p-3">
						<div className="flex items-center gap-2 flex-nowrap overflow-hidden">
							...
							>
								...
								<span className="tabular-nums">{n(stat.goles_totales)}</span>
							</Badge>
							...
							>
								...
								<span className="tabular-nums">{derived.efficiency}%</span>
							</Badge>
							...
							>
								...
								<span className="tabular-nums">{derived.totalActions}</span>
							</Badge>
						</div>
					</div>
				</div>
