...
type RivalShot = {
	id: number;
	// En tu query del server: rivalPenaltyShots filtra player_id === null y lleva `scored` y/o `result_type`
	scored?: boolean | null;
		...
		<div className="relative overflow-hidden rounded-xl border bg-card">
			...
			<div className="relative flex items-stretch">
				...
				<div className="shrink-0 w-[84px] sm:w-[104px] h-[84px] sm:h-[92px] overflow-hidden">
					{photoUrl ? (
						<img src={photoUrl || "/placeholder.svg"} alt={title} className="h-[140%] w-full object-cover object-top" loading="lazy" />
					) : (
						<div className="h-full w-full flex items-center justify-center bg-muted/40 border-r border-border/60">
							<span className="text-foreground/70 font-extrabold text-xl tabular-nums">{number ?? "—"}</span>
						</div>
					)}
				</div>
					...
					<div className="space-y-3">
						{rivalPenaltyShots.map((shot, idx) => {
							const result: "scored" | "saved" | "missed" =
								shot.result_type === "scored"
									? "scored"
									: shot.result_type === "saved"
										? "saved"
										: shot.result_type === "missed"
											? "missed"
											: shot.scored
												? "scored"
												: "missed";
