...
// cada lanzamiento es una fila
export type PenaltyShooter = { id: number; playerId: number; scored: boolean };
export type RivalPenalty = { id: number; result: "scored" | "saved" | "missed" };
								...
								{penaltyShooters.length === 0 ? (
									...
									<div className="space-y-2">
										{penaltyShooters.map((shooter, idx) => {
											...
											return (
												<div key={shooter.id} className={`rounded-lg border ${stateStyles}`}>
																...
																<button
																	...
																	onClick={() =>
																		setPenaltyShooters((prev) =>
																			prev.map((s) => (s.id === shooter.id ? { ...s, scored: !s.scored } : s))
																		)
																	}
								...
								{rivalPenalties.length === 0 ? (
									...
									<div className="space-y-2">
										{rivalPenalties.map((penalty, index) => {
											...
											const needsGoalkeeper = isSaved && !penaltyGoalkeeperMap[penalty.id];
																		...
																		<Select
																			value={
																				penaltyGoalkeeperMap[penalty.id]
																					? String(penaltyGoalkeeperMap[penalty.id])
																					: ""
																			}
																			onValueChange={(value) => {
																				const gkId = Number(value);
																				if (gkId)
																					setPenaltyGoalkeeperMap((prev) => ({
																						...prev,
																						[penalty.id]: gkId
