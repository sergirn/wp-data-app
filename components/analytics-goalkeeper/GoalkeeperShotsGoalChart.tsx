...
type ShotLayer = "all" | "goals" | "saves";
  ...
  return (
    <div className="rounded-xl border bg-background/40 p-3">
      <div className="mb-2 flex items-center gap-2">
        <div className="inline-flex h-7 w-7 items-center justify-center rounded-lg border bg-background">{icon ?? <div className="h-3.5 w-3.5" />}</div>
...
export function GoalkeeperShotsGoalChart({
  ...
  /** ====== STATS: totales base (sin capa) y visibles (con capa) ====== */
  const totalsAll = React.useMemo(() => {
    ...
    return { total: baseShots.length, goals, saves };
  }, [baseShots]);
  ...
  const layerLabel =
    shotLayer === "all" ? "Todos" : shotLayer === "goals" ? `Goles (${totalsVisible.total})` : `Paradas (${totalsVisible.total})`;
            ...
            <div className="absolute left-[10%] right-[10%] top-[12%] bottom-[28%] overflow-hidden rounded-xl border bg-muted/10">
              ...
              {showGrid && (
                <div className="pointer-events-none absolute inset-0">
                  ...
                  {showHeatmap && (
                    <HeatmapCanvas enabled={showHeatmap} points={filteredShots.map((s) => ({ x: s.x, y: s.y }))} opacity={0.75} radiusPx={42} />
                  )}
              ...
              {!showHeatmap && filteredShots.map((s, i) => <Dot key={`${s.id}-${s.goalkeeperPlayerId}-${i}`} x={s.x} y={s.y} result={s.result} />)}
            </div>
          </div>
          ...
          <div className="p-3 space-y-3 lg:sticky lg:top-4">
            <SidebarSection title="Visualización" icon={<Grid3X3 className="h-4 w-4 text-muted-foreground" />}>
              <div className="space-y-2">
                {/* ✅ NUEVO: selector de capa */}
                <div className="grid grid-cols-3 gap-2">
                  <Button type="button" size="sm" variant={shotLayer === "all" ? "default" : "outline"} className="h-8" onClick={() => setShotLayer("all")}>
                    Todos
                  </Button>
            ...
            <SidebarSection title="Jornada" icon={<ListFilter className="h-4 w-4 text-muted-foreground" />}>
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  <Button type="button" size="sm" variant="outline" className="h-8 px-2" onClick={goPrev} disabled={!canPrev} title="Anterior">
                  ...
                  <Button type="button" size="sm" variant="outline" className="h-8 px-2" onClick={goNext} disabled={!canNext} title="Siguiente">
                            ...
                            >
                              ...
                              <span
                                className={cn(
                                  ...
                                  active ? "bg-foreground text-background border-foreground/30" : "bg-background text-muted-foreground"
                                )}
                              >
                                {active ? "ON" : "OFF"}
                              </span>
