...
export function GoalkeeperShotsGoalChart({
  ...
  // ✅ Capa (todos / goles / paradas)
  const filteredShots = React.useMemo(() => {
    if (shotLayer === "goals") return baseShots.filter((s) => s.result === "goal");
    if (shotLayer === "saves") return baseShots.filter((s) => s.result === "save");
    return baseShots;
  }, [baseShots, shotLayer]);

  // Totales base (sin capa) y visibles (con capa)
  ...
  const layerLabel = shotLayer === "all" ? "Todos" : shotLayer === "goals" ? "Goles" : "Paradas";
          ...
          <div className="flex flex-wrap items-center gap-1.5 justify-end">
            ...
            {shotLayer !== "all" ? (
              <Badge variant="outline" className="text-[11px] h-5 px-2">
                Vista: {layerLabel} ({totalsVisible.total})
              </Badge>
            ) : null}
          </div>
        </div>
          ...
          <div className="p-3 space-y-3 lg:sticky lg:top-4">
            <SidebarSection title="Visualización" icon={<Grid3X3 className="h-4 w-4 text-muted-foreground" />}>
              <div className="space-y-2">
                {/* ✅ NUEVO: selector de capa (igual que el otro componente) */}
