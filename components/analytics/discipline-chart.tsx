...
export function DisciplineChart({ matches, stats }: DisciplineChartProps) {
	...
	const matchData = useMemo(() => {
		return sortedMatches.map((match, index) => {
			...
			// ✅ exp_trans_def
			const expTransDef = matchStats.reduce((sum, s) => sum + (s.exp_trans_def || 0), 0);

			const jornadaNumber = match.jornada ?? index + 1;

			// ✅ exp_trans_def sumado a exclusiones
			const totalExclusiones = exp20_1c1 + exp20_boya + expSimple + expTransDef;
	...
	const totalExpTransDef = matchData.reduce((sum, m) => sum + (m.expTransDef || 0), 0);

	const totalExclusiones = total20_1c1 + total20_boya + totalExpSimple + totalExpTransDef;
				...
				<div className="space-y-3">
					...
					<ChartContainer
						config={{
							...
							expTransDef: { label: "Exp. trans def", color: "hsl(200, 85%, 55%)" }
						}}
					...
					>
						<ResponsiveContainer width="100%" height="100%">
							<ComposedChart data={chartData} margin={{ top: 8, right: 14, left: 0, bottom: 0 }}>
								...
								<Bar yAxisId="left" dataKey="exp20_1c1" fill="var(--color-exp20_1c1)" name="Exp. 20s (1c1)" radius={[4, 4, 0, 0]} />
								<Bar
									yAxisId="left"
									dataKey="exp20_boya"
									...
									radius={[4, 4, 0, 0]}
								/>
								<Bar yAxisId="left" dataKey="expSimple" fill="var(--color-expSimple)" name="Exp. simple" radius={[4, 4, 0, 0]} />
								<Bar yAxisId="left" dataKey="penaltis" fill="var(--color-penaltis)" name="Penaltis" radius={[4, 4, 0, 0]} />
								<Bar
									yAxisId="left"
									dataKey="expTransDef"
									fill="var(--color-expTransDef)"
									name="Exp. trans def"
									radius={[4, 4, 0, 0]}
								/>
							...
							<Table className="min-w-[920px]">
								<UITableHeader className="sticky top-0 z-10 bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/75">
									<TableRow className="hover:bg-transparent">
										...
										<TableHead className="text-right hidden md:table-cell">Exp trans. def.</TableHead>
									...
									{matchData.map((m, idx) => {
										...
										return (
											...
											>
												...
												<TableCell className="text-right hidden md:table-cell tabular-nums">{m.expTransDef}</TableCell>
